...
def Generator(
    ...
    if atlas_model == 'voxelmorph' and conditional is False:
        ...
        condn_emb_vxm = KL.Dense((np.prod(input_resolution[:-1])), activation='elu')(condn)
        condn_emb_vxm = KL.Reshape([x//2 for x in input_resolution[:-1]] + [8])(condn_emb_vxm)
        ...
        dec_out = conv_dec(
            ...
            8,
            [x//2 for x in input_resolution[:-1]] + [8],
            2,
            [3, 3, 3],
            8,
        )
        ...
        # condn_emb_vxm = KL.Reshape((80, 96, 80, 8))(condn_emb_vxm)
        condn_emb_vxm = KL.Reshape(tuple([x//2 for x in input_resolution[:-1]] + [8]))(condn_emb_vxm)
        ...
        dec_out = conv_dec_film(
            ...
            8,
            # (80, 96, 80, 8),
            tuple([x//2 for x in input_resolution[:-1]] + [8]),
            2,
            [3, 3, 3],
            8,
            sn=True,
